...
def _is_tcp_port(port: str) -> bool:
    return port.startswith(("socket://", "tcp://"))


def _setup_tcp_port(self, cflag_baud):  # noqa: N802
    """Open socket://host:port with TCP_NODELAY. Baud is only used for packet timing."""
    import time
    ...
    if url.startswith("tcp://"):
        url = "socket://" + url[len("tcp://") :]
    # pyserial's socket handler rejects 1000000 baud and then ignores baud anyway.
    self.ser = serial.serial_for_url(url, baudrate=115200, timeout=0, do_not_open=False)
    sock = getattr(self.ser, "_socket", None)
    if sock is not None:
        sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    self.is_open = True
    # Phone bridge opens the USB serial only after the TCP client connects (async).
    # Wait briefly so the first Homing_Offset / ping is not lost.
    time.sleep(1.0)
    try:
    ...
    # Wi-Fi RTT needs a looser packet timeout than a local USB tty.
    self.setPacketTimeout = _patch_tcp_packet_timeout.__get__(self, type(self))  # type: ignore[method-assign]
    return True


def _patch_tcp_packet_timeout(self, packet_length):  # noqa: N802
    self.packet_start_time = self.getCurrentTime()
...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def __init__(
        ...
        if _is_tcp_port(self.port):
            # Raw TCP, not a PTY. socat's tty layer adds termios latency that misses 30 Hz.
            self.port_handler.setupPort = _setup_tcp_port.__get__(  # type: ignore[method-assign]
                self.port_handler, scs.PortHandler
            )
