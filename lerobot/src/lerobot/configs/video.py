...
def parse_video_orient(spec: str | None) -> list[str]:
    """Parse a transcode orient string into canonical ops, applied left to right."""
    if spec is None:
        return []
    text = str(spec).strip().lower()
    if text in ("", "none", "identity", "0"):
        return []
    ops: list[str] = []
    for part in text.replace("+", ",").replace(" ", ",").split(","):
        ...
        if not token:
            continue
        canonical = _ORIENT_ALIASES.get(token)
        if canonical is None:
            known = "rot90, rot180, rot270, hflip, vflip"
            raise ValueError(f"Unknown orient '{token}'. Use {known}, separated by commas.")
        ops.append(canonical)
    return ops


def orient_swaps_wh(ops: list[str]) -> bool:
    swap = False
    for op in ops:
...
class VideoEncoderConfig:
    ...
    # Applied only when transcoding to MP4. Comma-separated, left to right.
    # rot90 / rot180 / rot270 are clockwise. hflip / vflip mirror.
    # Example: "rot180,hflip". Empty leaves frames unchanged.
    orient: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        parse_video_orient(self.orient)
        self.validate()
