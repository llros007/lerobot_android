...
class DatasetWriter:
    ...
    def __init__(
        ...
        self._deferred_videos: list[tuple[str, int, Path]] = []
        self._ready_mp4s: list[tuple[str, int, Path]] = []
        self._transcode_thread: threading.Thread | None = None
        self._transcode_error: BaseException | None = None
    ...
    def save_episode(
        ...
        if use_streaming:
            ...
            for video_key in self._meta.video_keys:
                ...
                if Path(temp_path).suffix == ".mkv":
                    ep_idx = int(np.asarray(episode_index).reshape(-1)[0])
                    self._deferred_videos.append((video_key, ep_idx, Path(temp_path)))
                    continue
    ...
    def start_episode_transcode(self) -> None:
        """Transcode this episode's MKV files in the background. Do not concatenate yet."""
        self.wait_episode_transcode()
        jobs = self._deferred_videos
        self._deferred_videos = []
        if not jobs:
            return
        logger.info("本集 %s 路视频开始转 MP4，这段时间可以复位机械臂", len(jobs))
        self._transcode_error = None
        self._transcode_thread = threading.Thread(
            target=self._transcode_jobs,
            args=(jobs,),
            name="episode-mp4",
            daemon=True,
        )
    ...
    def episode_transcode_done(self) -> bool:
        thread = self._transcode_thread
        return thread is None or not thread.is_alive()

    def wait_episode_transcode(self) -> None:
        thread = self._transcode_thread
        if thread is not None:
            thread.join()
            self._transcode_thread = None
        if self._transcode_error is not None:
            error = self._transcode_error
            self._transcode_error = None
            raise error

    def _transcode_jobs(self, jobs: list[tuple[str, int, Path]]) -> None:
        import av

        previous_level = av.logging.get_level()
        av.logging.set_level(av.logging.ERROR)
        try:
            ready: list[tuple[str, int, Path]] = []
            workers = min(2, len(jobs))
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
                futures = [executor.submit(self._transcode_one, *job) for job in jobs]
                for future in futures:
                    ready.append(future.result())
            ready.sort(key=lambda item: (item[1], item[0]))
            self._ready_mp4s.extend(ready)
        except BaseException as exc:
            self._transcode_error = exc
        finally:
            av.logging.set_level(av.logging.WARNING if previous_level is None else previous_level)

    def _transcode_one(self, video_key: str, episode_index: int, mkv_path: Path) -> tuple[str, int, Path]:
        mp4_path = mkv_path.with_suffix(".mp4")
        encoder = self._depth_encoder if video_key in self._meta.depth_keys else self._rgb_encoder
        logger.info("转码 %s episode %s", video_key, episode_index)
        transcode_mkv_to_mp4(
            mkv_path,
            mp4_path,
            self._meta.fps,
        ...
        logger.info("转码完成 %s episode %s", video_key, episode_index)
        return video_key, episode_index, mp4_path

    def _finish_deferred_videos(self) -> None:
        """Wait for per-episode MP4s, then concatenate them into the dataset files."""
        if self._deferred_videos:
            jobs = self._deferred_videos
            self._deferred_videos = []
            self._transcode_jobs(jobs)
        self.wait_episode_transcode()
        jobs = self._ready_mp4s
        self._ready_mp4s = []
        if not jobs:
            return
        logger.info("全部转码结束，开始拼接 %s 段视频", len(jobs))
        patched: set[int] = set()
        pending_disk: list[tuple[int, dict]] = []
        for video_key, episode_index, mp4_path in jobs:
            metadata = self._save_episode_video(video_key, episode_index, temp_path=mp4_path)
            self._note_video_for_next_episode(metadata)
            if self._patch_episode_video_metadata(episode_index, metadata):
                patched.add(episode_index)
            else:
                pending_disk.append((episode_index, metadata))
        if pending_disk:
            self._meta._close_writer()
            for episode_index, metadata in pending_disk:
                if episode_index in patched:
                    continue
                self._rewrite_episode_video_metadata(episode_index, metadata)

    def _transcode_deferred_videos(self) -> None:
        self._finish_deferred_videos()

    def _note_video_for_next_episode(self, metadata: dict) -> None:
        """Keep the latest episode's video span so the next episode appends.

        Deferred transcode runs after every episode is already saved, so
        ``latest_episode`` is the last episode and does not yet have video
        keys. Without this, each episode is treated as a new file and
        overwrites ``file-000.mp4``.
        """
        latest = self._meta.latest_episode
        if not isinstance(latest, dict):
            return
        for key, value in metadata.items():
            if key == "episode_index" or not str(key).startswith("videos/"):
                continue
            latest[key] = value if isinstance(value, list) else [value]

    def _patch_episode_video_metadata(self, episode_index: int, metadata: dict) -> bool:
        updates = {k: v for k, v in metadata.items() if k != "episode_index"}
        buf = getattr(self._meta, "_metadata_buffer", [])
        for ep in buf:
            idx = ep.get("episode_index")
            idx = idx[0] if isinstance(idx, list) else idx
            if int(idx) != int(episode_index):
                continue
            for key, value in updates.items():
                ep[key] = [value]
            latest = self._meta.latest_episode
            if latest is not None:
                latest_idx = latest.get("episode_index")
                latest_idx = latest_idx[0] if isinstance(latest_idx, list) else latest_idx
                if int(latest_idx) == int(episode_index):
                    self._meta.latest_episode = ep
            return True
        return False

    def _rewrite_episode_video_metadata(self, episode_index: int, metadata: dict) -> None:
        updates = {k: v for k, v in metadata.items() if k != "episode_index"}
        for path in sorted(self._meta.root.glob("meta/episodes/**/*.parquet")):
            ...
            return
        logger.warning("找不到 episode %s 的元数据，视频时间戳没有写回去", episode_index)
    ...
    def flush_pending_videos(self) -> None:
        ...
        """
        if self._deferred_videos or self._ready_mp4s or self._transcode_thread is not None:
            self._finish_deferred_videos()
        if self._streaming_encoder is not None:
