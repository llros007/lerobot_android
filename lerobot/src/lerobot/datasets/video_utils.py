...
from lerobot.configs.video import orient_swaps_wh, parse_video_orient
...
def _jpeg_size(data: bytes) -> tuple[int, int]:
    """Read width and height from a JPEG SOF marker. Does not decode the image."""
    i = 2
    n = len(data)
    while i + 8 < n:
        if data[i] != 0xFF:
        ...
        if marker in (0xC0, 0xC1, 0xC2) and i + 7 <= n:
            ...
            return width, height
        i += seglen
    raise ValueError(f"JPEG of {n} bytes has no SOF marker")


def _mux_encoded(container, packets) -> None:
    if not packets:
        return
...
def _apply_orient(frame, ops: list[str]):
    """Rotate or flip one decoded frame. Ops are canonical names from parse_video_orient."""
    if not ops:
        return frame
    ...
    return av.VideoFrame.from_ndarray(np.ascontiguousarray(image), format="rgb24")
...
def transcode_mkv_to_mp4(
    ...
    encoder_threads: int | None = None,
    *,
    manage_log_level: bool = True,
) -> None:
    """Decode an MJPEG Matroska file and write an MP4 with the dataset encoder."""
    # JPEG is full-range YUV. swscale warns once per frame unless this is ERROR.
    previous_level = av.logging.get_level() if manage_log_level else None
    if manage_log_level:
        av.logging.set_level(av.logging.ERROR)
    inp = av.open(str(src))
    try:
        in_stream = inp.streams.video[0]
        ops = parse_video_orient(getattr(video_encoder, "orient", ""))
        width = in_stream.codec_context.width
        height = in_stream.codec_context.height
        if orient_swaps_wh(ops):
            width, height = height, width
        if ops:
            logger.info("MP4 orient %s (%sx%s)", ",".join(ops), width, height)
        dst.parent.mkdir(parents=True, exist_ok=True)
        out = av.open(str(dst), "w")
        try:
            out_stream = out.add_stream(
                video_encoder.vcodec,
                fps,
                options=video_encoder.get_codec_options(encoder_threads, as_strings=True),
            )
            out_stream.pix_fmt = video_encoder.pix_fmt
            out_stream.width = width
            out_stream.height = height
            ...
            for frame in inp.decode(in_stream):
                if ops:
                    frame = _apply_orient(frame, ops)
                video_frame = frame.reformat(format=video_encoder.pix_fmt)
                video_frame.pts = count
                video_frame.time_base = Fraction(1, fps)
                _mux_encoded(out, out_stream.encode(video_frame))
                count += 1
            _mux_encoded(out, out_stream.encode())
        finally:
            out.close()
    finally:
        inp.close()
...
class _CameraEncoderThread(threading.Thread):
    """A thread that writes camera frames streamed via a queue.

    JPEG bytes are muxed into MKV as-is (no decode). Numpy frames are encoded to MP4.
    One instance is created per camera per episode.
    """
    ...
    def __init__(
        ...
        self._jpeg_only = False
        self._jpeg_pts = 0
        self._raw = None
        self._raw_path: Path | None = None
        self._jpeg_wh: tuple[int, int] | None = None

    def _mux_jpeg(self, data: bytes) -> None:
        """Append one JPEG. Matroska is built after the episode so PyAV stays off the 30 Hz loop."""
        if self._raw is None:
            width, height = _jpeg_size(data)
            self._jpeg_wh = (width, height)
            raw_path = Path(self.video_path).with_suffix(".jseq")
            raw_path.parent.mkdir(parents=True, exist_ok=True)
            self._raw_path = raw_path
            self._raw = open(raw_path, "wb", buffering=8 * 1024 * 1024)
            self.video_path = Path(self.video_path).with_suffix(".mkv")
            self._jpeg_only = True
            logger.info("Buffering JPEG for %s (%sx%s)", self.video_path.name, width, height)
        self._raw.write(len(data).to_bytes(4, "little"))
        self._raw.write(data)
        self._jpeg_pts += 1

    def _seal_jpeg_mkv(self) -> None:
        raw = self._raw
        self._raw = None
        if raw is None or self._raw_path is None or self._jpeg_wh is None:
            return
        raw.flush()
        raw.close()
        width, height = self._jpeg_wh
        previous_level = av.logging.get_level()
        av.logging.set_level(av.logging.ERROR)
        container = av.open(str(self.video_path), "w", format="matroska")
        try:
            stream = container.add_stream("mjpeg", rate=self.fps)
            stream.width = width
            stream.height = height
            stream.pix_fmt = "yuvj420p"
            stream.time_base = Fraction(1, self.fps)
            pts = 0
            with open(self._raw_path, "rb") as src:
                while True:
                    header = src.read(4)
                    if len(header) < 4:
                        break
                    ...
                    container.mux(packet)
                    pts += 1
            logger.info("Sealed %s frames into %s", pts, self.video_path.name)
        finally:
            container.close()
            av.logging.set_level(av.logging.WARNING if previous_level is None else previous_level)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                jpeg_bytes = frame_data if isinstance(frame_data, (bytes, bytearray)) else None
                decoded = None
                if jpeg_bytes is not None:
                    if self.is_depth:
                        raise RuntimeError("JPEG passthrough is not supported for depth cameras")
                    try:
                        self._mux_jpeg(bytes(jpeg_bytes))
                    except ValueError as exc:
                        logger.warning("skip bad JPEG (%s bytes): %s", len(jpeg_bytes), exc)
                        continue
                    frame_count += 1
                    continue
                ...
                if decoded is not None:
                    video_frame = decoded.reformat(format=self.video_encoder.pix_fmt)
            ...
            if self._jpeg_only or frame_count < 2:
...
class StreamingVideoEncoder:
    ...
    def feed_frame(self, video_key: str, image: np.ndarray) -> None:
        ...
        try:
            if isinstance(image, bytes):
                ...
                payload = image.copy()
            self._frame_queues[video_key].put_nowait(payload)
        except queue.Full:
    ...
    def finish_episode(self) -> dict[str, tuple[Path, dict | None]]:
        ...
        for video_key in self._threads:
            ...
            self._video_paths[video_key] = self._threads[video_key].video_path
