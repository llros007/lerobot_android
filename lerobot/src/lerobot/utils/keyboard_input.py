...
def init_keyboard_listener(ignore_letters: set[str] | None = None):
    ...
    * otherwise no listener (non-interactive / piped runs) — recording relies on
    ...
    skipped = {letter.lower() for letter in (ignore_letters or set())}
    ...
    letters = [label for letter, label in (("n", "n=next"), ("r", "r=re-record"), ("q", "q=quit")) if letter not in skipped]
    listener = create_key_listener(
        on_key, controls_help="Right/Left/Esc" + ("".join(f", {label}" for label in letters))
    )
