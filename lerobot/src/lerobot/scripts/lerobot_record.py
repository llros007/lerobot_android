...
from lerobot.cameras.mjpeg import MjpegCameraConfig  # noqa: F401
...
def record_loop(
    ...
    display_mode: str = "rerun",
    display_compressed_images: bool = False,
    keyboard: Teleoperator | None = None,
):
    ...
    while timestamp < control_time_s:
        ...
        if isinstance(teleop, Teleoperator):
            ...
            if keyboard is not None and hasattr(robot, "_from_keyboard_to_base_action"):
        ...
        # XLerobot remaps leader keys (shoulder_pan.pos -> arm_shoulder_pan.pos)
        # and fills wheel velocities. Save that action so it matches dataset features.
        if hasattr(robot, "_prefix_arm_action"):
...
def record(
    ...
    try:
        ...
        if hasattr(robot, "_from_keyboard_to_base_action"):
            ...
            keyboard = KeyboardTeleop(KeyboardTeleopConfig(id=f"{robot.id}_keyboard"))
            keyboard.connect()
            keys = getattr(robot, "teleop_keys", None) or {}
            logging.info(
                "Keyboard base control: forward=%s backward=%s rotate_left=%s "
                "rotate_right=%s speed_up=%s speed_down=%s (focus this terminal)",
                keys.get("forward", "i"),
                keys.get("backward", "k"),
                keys.get("rotate_left", "u"),
                keys.get("rotate_right", "o"),
                keys.get("speed_up", "n"),
                keys.get("speed_down", "m"),
            )
        robot.connect()

        base_letters = set((getattr(robot, "teleop_keys", None) or {}).values()) if keyboard else set()
        listener, events = init_keyboard_listener(ignore_letters=base_letters & {"n", "r", "q"})
        ...
        with VideoEncodingManager(dataset):
            # Release the arm before __exit__ transcodes. Official streaming encode
            # finishes inside the control loop, so torque drops as soon as recording
            # stops. Deferred MKV→MP4 would otherwise hold the last goal for minutes.
            try:
                recorded_episodes = 0
                while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                    ...
                    if dataset.writer is not None:
                        dataset.writer.start_episode_transcode()
                    recorded_episodes += 1

                    # Transcode overlaps the reset. The arm keeps following the leader
                    # until this episode's MP4s are done, so the next episode does not
                    # share the CPU with AV1. Concatenation happens once, at the end.
                    if not events["stop_recording"] and recorded_episodes < cfg.dataset.num_episodes:
                        log_say("Reset the environment", cfg.play_sounds)
                        logging.info("本集正在转 MP4，可以复位机械臂，转完再录下一集")
                        reset_started = time.perf_counter()
                        min_reset_s = float(cfg.dataset.reset_time_s)
                        while not events["stop_recording"]:
                            ...
                            elapsed = time.perf_counter() - reset_started
                            transcode_done = (
                                dataset.writer is None or dataset.writer.episode_transcode_done()
                            )
                            if transcode_done and elapsed >= min_reset_s:
                                break
                        if dataset.writer is not None and not events["stop_recording"]:
                            dataset.writer.wait_episode_transcode()
                        events["rerecord_episode"] = False
                        events["exit_early"] = False
            finally:
