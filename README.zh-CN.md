...
用闲置手机当机器人上的电脑，代替 LeKiwi 等移动机器人上的树莓派。现在可以遥操单臂 XLeRobot、用键盘开底盘，并把一集数据录成 LeRobot 数据集。手机负责从臂总线和摄像头，电脑负责主臂以及 `lerobot-teleoperate` / `lerobot-record`。最终在手机上运行策略推理。

![手机装在机器人上](media/phone-on-robot.gif)

手机装在机器人上，屏幕显示眼睛。从臂串口和腕部摄像头插在手机上。电脑把前置相机、腕部相机、手臂和底盘录在一起：

![录制的前置和腕部画面](media/record-front-wrist.gif)
